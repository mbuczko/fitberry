    ...
    $.fn.extend({
        ...
        _loadContents: function(param) {
            ...
            if (param.url.match(re)) {
                ...
                var self = this;
                $('#simple-modal .simple-modal-body').load(param.url, function(responseText, textStatus, XMLHttpRequest) {
                    var container = $(this).parent().removeClass("loading").removeClass("hide-footer");
                    if (textStatus !== 'success') {
                        container.find(".contents").html("loading failed");

                        if (param.onRequestFailure) {
                            param.onRequestFailure.call(self);
                        }
                    }
                    else
                    {
                        if (param.onRequestComplete) {
                            param.onRequestComplete.call(self);
                        }
                        self._display();
                    }
                });
